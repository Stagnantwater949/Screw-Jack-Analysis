...
% Define a range of loads
loads = 0:10:100; % Load range from 0 to 100 KN in steps of 10
torque_values = zeros(size(loads));

for i = 1:length(loads)
    W = loads(i);
    T1 = (W * 1000 * tan(e) * (dm / 2)); % Calculate torque for each load
    torque_values(i) = T1;
end

% Plotting Torque vs. Load
figure;
plot(loads, torque_values, '-o', 'LineWidth', 2);
xlabel('Load (KN)');
ylabel('Torque Required (Nmm)');
title('Torque vs. Load for Screw Jack');
grid on;

% Safety factors
safety_factors = [Sigma_S / tau, Sigma_S / sigmaC, Tau_nut_safe / Tau_nut];
safety_labels = {'Safety Factor (Screw Shear)', 'Safety Factor (Screw Compressive)', 'Safety Factor (Nut Shear)'};

% Plotting Safety Factors
figure;
bar(safety_factors);
set(gca, 'xticklabel', safety_labels);
ylabel('Safety Factor');
title('Safety Factors for Screw Jack Components');
grid on;
